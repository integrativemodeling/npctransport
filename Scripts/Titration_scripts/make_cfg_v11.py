...
for i in rrange:
    ...
    if n_kaps>0:
        ...
        pos= kaps[i].site_coordinates.add()
        pos.x= 1.0*i
        pos.y= 0.0*i
        pos.z= 0.0*i
        pos= kaps[i].site_coordinates.add()
        pos.x= math.cos(0.666667*math.pi)*i # 120 degrees from site #1
        pos.y= math.sin(0.666667*math.pi)*i
        pos.z= 0.0
        ...
        create_range(interaction.interaction_k, 1.1*kap_k, 1.3*kap_k, steps=3, base=1)
        create_range(interaction.interaction_range, 1.1*kap_range, 1.3*kap_range, steps=3, base=1)
