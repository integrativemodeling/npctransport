...
if(len(sys.argv)>1):
  SKIP_ROWS= sys.argv[1]
else:
  SKIP_ROWS=100
for fg in  ["FSFG_generic", "GLFG_generic", "Nup1", "Nup100", "Nup49", "Nsp1"]:
  cmd_n_fgs= "cat config_{fg}.txt | ".format(fg=fg) + \
        " awk 'BEGIN{is=0}{if(is==1){print $NF; is=2; }" + \
        " else { if($1 ~ /number_of_b/){is=1}}}'"
  n_fgs= int( subprocess.check_output(cmd_n_fgs, shell=True) )
  Rbead=8.0
  output= Output()
  fname= "output_{fg}.pb".format(fg=fg)
  with open(fname, "rb") as f:
    ...
    # for a sphere's Rg
    for op in stats.fgs[0].order_params[SKIP_ROWS:]:
