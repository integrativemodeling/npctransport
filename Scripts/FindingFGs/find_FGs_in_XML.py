...
def get_options():
    ...
    parser.add_option("-n", "--include_nups",
                      action="store_true", dest="include_nups",
                      help="if specified, include known nucleoporins in output")
...
for event, elem in context:
    ...
    # extract sequence of each entry and print fg sequences info
            ...
            is_nup= not re.search("nup|nuclear pore complex|nucleoporin",
                                  full_prot_name,
                                  re.IGNORECASE)
            if util.is_fg(sequence) and (options.include_nups or not is_nup):
