...
OBSTACLE_SCALE_FACTOR=1.1 # inflate obstacles a bit to prevent artificial cavities
kap_k=4.75
kap_range=4.5
sigma0_deg=45
sigma1_deg=45
...
def get_fgs_regions_to_params(is_remove_gle1_and_nup42):
    ...
    fgs_regions_to_params['Nup1']= {
        ...
        'm': default_fgp.get_copy(326, 797, # DOUBLE CHECK
                                  self_k= 1.55,
                                  nonspec_k= 0.04),
        'c': default_fgp.get_copy(798, 1076, # DOUBLE CHECK - this region has less charges (Timney et al.)
                                  self_k= 1.6,
                                  nonspec_k= 0.15)
    }
...
def get_coarse_grained_obstacles(obstacles):
    ...
    nn= IMP.algebra.NearestNeighbor3D(anchor_coords)
    in_obstacles= defaultdict(list)
    for (radius, coords) in obstacles.iteritems():
        for coord in coords:
            nearest_anchor= nn.get_nearest_neighbor(coord)
            dist_anchor= IMP.algebra.get_distance(coord, nearest_anchor)
            max_allowed_error_A= 1.5 * () dist_anchor/10.0)^1.5
            ...
            max_allowed_error_A= min(40.0, max_allowed_error_A)
