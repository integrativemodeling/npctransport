...
n_kaps=16
box_volume_L= n_kaps/AVOGADRO/args.kap_concentration
box_volume_A3= box_volume_L*1E27
BOX_SIDE_A= box_volume_A3**(1/3.0)
n_fg_chains_raw= box_volume_L*AVOGADRO*args.fg_concentration
n_fg_chains = int(round(n_fg_chains_raw)) if args.fg_concentration>0.0 else 0
fg_concentration_actual= n_fg_chains/AVOGADRO/box_volume_L
print "n_fg_chains", n_fg_chains, "n_kaps", n_kaps
print("FG nups concaentration %.3e [M]" % fg_concentration_actual)
print("Kap concaentration %.3e [M]" % args.kap_concentration)
...
def get_basic_config():
    ...
    config.excluded_volume_k.lower=5 # NO-DEBUG
...
for i in rrange:
    ...
    if n_kaps>0:
        ...
        create_range(interaction.interaction_k, .9*kap_k, 1.1*kap_k, steps=3, base=1)
        create_range(interaction.interaction_range, .9*kap_range, 1.1*kap_range, steps=3, base=1)
