...
n_fg_chains= 60
ratio_actual= math.ceil(ratio*n_fg_chains)/n_fg_chains
...
for modulus in [2,3,5,2,3,5,2,3,5]:
      while(n_kaps%modulus==0
            ...
            and min(n_kaps/modulus,n_fg_chains/modulus)>=1):
...
#   create_range(kaps[i].number, lb=1, ub=600, steps=40, base=1.1)
    ...
    create_range(interaction.interaction_k, 0.8*kap_k, 1.2*kap_k, steps=7)
    create_range(interaction.interaction_range, 0.75*kap_range, 1.25*kap_range, steps=7)
